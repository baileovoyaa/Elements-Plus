...
public class CrystallizerRecipe implements Recipe<CrystallizerRecipe.Input> {

    public static final ResourceLocation TYPE_ID = ElementsPlus.id("crystallizer");
    public static final RecipeType<CrystallizerRecipe> TYPE = Registry.register(BuiltInRegistries.RECIPE_TYPE, TYPE_ID, new RecipeType<CrystallizerRecipe>() {
        @Override
        public String toString() {
            return TYPE_ID.toString();
        }
    });
    public static final RecipeSerializer<CrystallizerRecipe> SERIALIZER = RecipeSerializer.register(TYPE_ID.toString(), new Serializer());
    ...
    public CrystallizerRecipe(List<InputEntry> inputs, ItemStack output) {
        this.inputs = inputs;
        this.output = output;
    }
    ...
    public static List<CrystallizerRecipe> getAll(Level level) {
        return level.getRecipeManager().getAllRecipesFor(TYPE).stream()
                .map(net.minecraft.world.item.crafting.RecipeHolder::value)
                .toList();
    }

    public static CrystallizerRecipe findRecipe(ItemStack inputStack, Level level) {
        List<ItemStack> stacks = new ArrayList<>();
        if (!inputStack.isEmpty()) {
            stacks.add(inputStack);
        }
        for (CrystallizerRecipe recipe : getAll(level)) {
            Input input = new Input(stacks);
    ...
    public static boolean canSmelt(ItemStack inputStack, Level level) {
        return findRecipe(inputStack, level) != null;
    }
    ...
    public static final class Serializer implements RecipeSerializer<CrystallizerRecipe> {
        ...
        public static final StreamCodec<RegistryFriendlyByteBuf, CrystallizerRecipe> STREAM_CODEC = StreamCodec.of(
                (buf, recipe) -> {
                ...
                (buf) -> {
                    ...
                    return new CrystallizerRecipe(inputs, output);
                }
        );
